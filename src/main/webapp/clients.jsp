...
<head>
  ...
  <title>Wheels - Manage Clients</title>
...
<div class="fixed top-0 left-0 h-full w-64 sidebar z-40">
  <div class="p-6">
    ...
    <nav class="space-y-2">
      ...
      <a href="clients" class="sidebar-item active flex items-center space-x-3 p-3 rounded-lg">
        <svg class="w-6 h-6 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<main class="ml-64 p-8 min-h-screen">
  ...
  <header class="flex flex-col md:flex-row justify-between items-center mb-8">
    <h1 class="text-3xl font-bold text-white mb-4 md:mb-0">Manage Clients</h1>
    <div class="flex items-center space-x-4">
      <button onclick="openModal('add')" class="btn-primary text-white px-4 py-2 rounded-lg">Add Client</button>
    </div>
  </header>
  ...
  <section class="card p-6">
    ...
    <c:if test="${empty clients}">
      <p class="text-gray-400">No clients found. Add a new client to get started.</p>
    </c:if>
        ...
        <table class="w-full text-left">
          ...
          <c:forEach var="client" items="${clients}">
            <tr class="border-t border-gray-700 table-row">
              <td class="text-white">CL-WS${client.user_id}</td>
              <td class="text-white">${client.full_name}</td>
              <td class="text-white">${client.email}</td>
              <td class="text-white">${client.phone}</td>
              <td class="text-white"><fmt:formatDate value="${client.created_at}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
              <td class="space-x-2">
                <button onclick="openModal('edit', ${client.user_id}, '${client.full_name}', '${client.email}', '${client.phone}')" class="text-blue-400 hover:text-blue-300 font-medium">Edit</button>
                <form action="clients" method="post" style="display:inline;">
                  <input type="hidden" name="action" value="delete">
                  <input type="hidden" name="userId" value="${client.user_id}">
                  <button type="submit" class="text-red-400 hover:text-red-300 font-medium" onclick="return confirm('Are you sure you want to delete this client?')">Delete</button>
                </form>
              </td>
            </tr>
          </c:forEach>
...
<div id="clientModal" class="modal">
  <div class="modal-content">
    ...
    <form id="clientForm" action="clients" method="post">
