...
<head>
    ...
    <title>Wheels - Manage Drivers</title>
...
<div class="fixed top-0 left-0 h-full w-64 sidebar z-40">
    <div class="p-6">
        ...
        <nav class="space-y-2">
            ...
            <a href="drivers" class="sidebar-item active flex items-center space-x-3 p-3 rounded-lg">
                <svg class="w-6 h-6 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<main class="ml-64 p-8 min-h-screen">
    ...
    <header class="flex flex-col md:flex-row justify-between items-center mb-8">
        <h1 class="text-3xl font-bold text-white mb-4 md:mb-0">Manage Drivers</h1>
        <div class="flex items-center space-x-4">
            <button onclick="openModal('add')" class="btn-primary text-white px-4 py-2 rounded-lg">Add Driver</button>
        </div>
    </header>
    ...
    <section class="card p-6">
        <c:if test="${empty drivers}">
            <p class="text-gray-400">No drivers found. Add a new driver to get started.</p>
        </c:if>
                ...
                <table class="w-full text-left">
                    ...
                    <c:forEach var="driver" items="${drivers}">
                        <tr class="border-t border-gray-700 table-row">
                            <td class="text-white">DR-${driver.user_id}</td>
                            <td class="text-white">${driver.full_name}</td>
                            <td class="text-white">${driver.email}</td>
                            <td class="text-white">${driver.phone}</td>
                            <td class="text-white"><fmt:formatDate value="${driver.created_at}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
                            <td class="space-x-2">
                                <button onclick="openModal('edit', ${driver.user_id}, '${driver.full_name}', '${driver.email}', '${driver.phone}')" class="text-blue-400 hover:text-blue-300 font-medium">Edit</button>
                                <form action="drivers" method="post" style="display:inline;">
                                    <input type="hidden" name="action" value="delete">
                                    <input type="hidden" name="userId" value="${driver.user_id}">
                                    <button type="submit" class="text-red-400 hover:text-red-300 font-medium" onclick="return confirm('Are you sure you want to delete this driver?')">Delete</button>
                                </form>
                            </td>
                        </tr>
                    </c:forEach>
        ...
        <form id="driverForm" action="drivers" method="post">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                <div>
                    ...
                    <input type="text" name="fullName" id="fullName" class="input-field" required>
                </div>
                <div>
...
<script>
    function openModal(action, userId = '', fullName = '', email = '', phone = '') {
