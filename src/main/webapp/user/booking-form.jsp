...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-28">
    <h1 class="text-4xl md:text-5xl font-bold text-[var(--charcoal-gray)] mb-10">Book Your Car</h1>
    <%
        if (userId == null) {
    %>
    <p class="text-[var(--charcoal-gray)] text-lg">Please <a href="${pageContext.request.contextPath}/auth" class="text-[var(--primary-red)] hover:underline">sign in</a> to book a car.</p>
    <% } else { %>
    <div class="bg-white shadow-lg rounded-xl p-8">
        <%
            ...
            if (carId == null) {
        %>
        <p class="text-[var(--charcoal-gray)] text-lg">No car selected. Please <a href="${pageContext.request.contextPath}/browse" class="text-[var(--primary-red)] hover:underline">select a car</a> to book.</p>
        <% } else { %>
        <%
        ...
        <% } %>
        <form action="${pageContext.request.contextPath}/book" method="post" class="space-y-6">
            <input type="hidden" name="carId" value="<%= carId %>">
            <div>
                <label for="startDate" class="block text-sm font-medium text-[var(--charcoal-gray)]">Pick-up Date</label>
                <input type="date" id="startDate" name="startDate" class="input-focus p-3 w-full" value="<%= LocalDate.now().plusDays(1).toString() %>" min="<%= LocalDate.now().plusDays(1).toString() %>" required aria-label="Select pick-up date">
            </div>
            <div>
                <label for="endDate" class="block text-sm font-medium text-[var(--charcoal-gray)]">Drop-off Date</label>
                <input type="date" id="endDate" name="endDate" class="input-focus p-3 w-full" value="<%= LocalDate.now().plusDays(2).toString() %>" min="<%= LocalDate.now().plusDays(2).toString() %>" required aria-label="Select drop-off date">
            </div>
            <div class="flex space-x-4">
                <button type="submit" class="btn-red font-semibold text-base py-2 px-6 rounded-lg">Confirm Booking</button>
                <a href="${pageContext.request.contextPath}/browse" class="btn-gold font-semibold text-base py-2 px-6 rounded-lg">Cancel</a>
            </div>
        </form>
        <% } %>
    </div>
