...
<head>
    ...
    <title>Wheels - Driver Bookings</title>
        ...
        <div class="flex justify-between h-20 items-center">
            <div class="flex items-center space-x-12">
                ...
                <div class="hidden md:flex space-x-8">
                    ...
                    <a href="${pageContext.request.contextPath}/profile" class="text-[var(--charcoal-gray)] hover:text-[var(--primary-red)] text-base font-medium transition-all duration-300 py-2 border-b-2 border-transparent hover:border-[var(--primary-red)]">Profile</a>
                    <a href="${pageContext.request.contextPath}/driver" class="text-[var(--primary-red)] text-base font-medium transition-all duration-300 py-2 border-b-2 border-[var(--primary-red)]">Driver Dashboard</a>
                </div>
            </div>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-28">
    <h1 class="text-4xl md:text-5xl font-bold text-[var(--charcoal-gray)] mb-10">Driver Dashboard</h1>
    <%
        if (userId == null) {
    %>
    <p class="text-[var(--charcoal-gray)] text-lg">Please <a href="${pageContext.request.contextPath}/auth" class="text-[var(--primary-red)] hover:underline">sign in</a> to view your driver dashboard.</p>
    <% } else if (!"driver".equals(SessionUtil.getUserRole(session))) { %>
    <p class="text-[var(--charcoal-gray)] text-lg">You do not have access to this page. Please <a href="${pageContext.request.contextPath}/" class="text-[var(--primary-red)] hover:underline">return to the homepage</a>.</p>
    <% } else { %>
    <%
    ...
    %>
    <p class="text-[var(--charcoal-gray)] text-lg">No bookings assigned to you at the moment.</p>
    <% } else { %>
    <div class="bg-white shadow-lg rounded-xl p-6">
        <table class="w-full text-left">
            <thead>
            <tr class="border-b border-gray-200">
                <th class="py-3 px-4 text-[var(--charcoal-gray)] font-semibold">Booking ID</th>
                <th class="py-3 px-4 text-[var(--charcoal-gray)] font-semibold">Car Model</th>
                <th class="py-3 px-4 text-[var(--charcoal-gray)] font-semibold">Customer</th>
                <th class="py-3 px-4 text-[var(--charcoal-gray)] font-semibold">Start Date</th>
                <th class="py-3 px-4 text-[var(--charcoal-gray)] font-semibold">End Date</th>
                <th class="py-3 px-4 text-[var(--charcoal-gray)] font-semibold">Status</th>
                <th class="py-3 px-4 text-[var(--charcoal-gray)] font-semibold">Action</th>
            </tr>
            </thead>
            <tbody>
            <% for (Map<String, Object> booking : bookings) { %>
            <tr class="border-b border-gray-200 hover:bg-gray-50 transition-colors duration-200">
                <td class="py-3 px-4"><%= booking.get("booking_id") %></td>
                <td class="py-3 px-4"><%= booking.get("model") %></td>
                <td class="py-3 px-4"><%= booking.get("customer_name") %></td>
                <td class="py-3 px-4"><%= booking.get("start_date") %></td>
                <td class="py-3 px-4"><%= booking.get("end_date") %></td>
                <td class="py-3 px-4"><%= booking.get("status") %></td>
                <td class="py-3 px-4">
                    <% if ("pending".equals(booking.get("status"))) { %>
                    <form action="${pageContext.request.contextPath}/driver" method="post" style="display:inline;">
                        <input type="hidden" name="bookingId" value="<%= booking.get("booking_id") %>">
                        <button type="submit" name="action" value="accept" class="btn-gold text-[var(--charcoal-gray)] font-semibold text-sm py-1 px-3 rounded-lg transition-transform hover:scale-105" onclick="return confirm('Are you sure you want to accept this booking?');">Accept</button>
                    </form>
                    <% } else if ("active".equals(booking.get("status"))) { %>
                    <form action="${pageContext.request.contextPath}/driver" method="post" style="display:inline;">
                        <input type="hidden" name="bookingId" value="<%= booking.get("booking_id") %>">
                        <button type="submit" name="action" value="complete" class="btn-red text-white font-semibold text-sm py-1 px-3 rounded-lg transition-transform hover:scale-105" onclick="return confirm('Are you sure you want to mark this booking as completed?');">Complete</button>
                    </form>
                    <% } else { %>
