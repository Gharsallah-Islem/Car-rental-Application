...
<head>
    ...
    <title>Wheels - My Bookings</title>
        ...
        <div class="flex justify-between h-20 items-center">
            <div class="flex items-center space-x-12">
                ...
                <div class="hidden md:flex space-x-8">
                    ...
                    <a href="${pageContext.request.contextPath}/my-bookings" class="text-[var(--primary-red)] text-base font-medium transition-all duration-300 py-2 border-b-2 border-[var(--primary-red)]">My Bookings</a>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-28">
    <h1 class="text-4xl md:text-5xl font-bold text-[var(--charcoal-gray)] mb-10">My Bookings</h1>
    <%
        if (userId == null) {
    %>
    <p class="text-[var(--charcoal-gray)] text-lg">Please <a href="${pageContext.request.contextPath}/auth" class="text-[var(--primary-red)] hover:underline">sign in</a> to view your bookings.</p>
    <% } else { %>
    <%
    ...
    %>
    <p class="text-[var(--charcoal-gray)] text-lg">You have no bookings yet. <a href="${pageContext.request.contextPath}/browse" class="text-[var(--primary-red)] hover:underline">Browse cars</a> to get started!</p>
    <% } else { %>
    <div class="bg-white shadow-lg rounded-xl p-6">
        <table class="w-full text-left">
            ...
            <tr class="border-b border-gray-200">
                ...
                <th class="py-3 px-4 text-[var(--charcoal-gray)] font-semibold">Total Cost</th>
            ...
            <tr class="border-b border-gray-200 hover:bg-gray-50 transition-colors duration-200">
                ...
                <td class="py-3 px-4">$<%= String.format("%.2f", booking.get("total_cost")) %></td>
                <td class="py-3 px-4"><%= booking.get("status") %></td>
                <td class="py-3 px-4">
                    <% if ("pending".equals(booking.get("status")) || "active".equals(booking.get("status"))) { %>
                    <form action="${pageContext.request.contextPath}/my-bookings" method="post" style="display:inline;">
                        <input type="hidden" name="bookingId" value="<%= booking.get("booking_id") %>">
                        <button type="submit" name="action" value="cancel" class="btn-red text-white font-semibold text-sm py-1 px-3 rounded-lg transition-transform hover:scale-105" onclick="return confirm('Are you sure you want to cancel this booking?');">Cancel</button>
                    </form>
                    <% } else { %>
