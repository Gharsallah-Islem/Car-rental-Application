...
<head>
    ...
    <title>Wheels - Manage Parcs</title>
...
<div class="fixed top-0 left-0 h-full w-64 sidebar z-40">
    <div class="p-6">
        ...
        <nav class="space-y-2">
            ...
            <a href="parcs" class="sidebar-item active flex items-center space-x-3 p-3 rounded-lg">
                <svg class="w-6 h-6 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<main class="ml-64 p-8 min-h-screen">
    ...
    <header class="flex flex-col md:flex-row justify-between items-center mb-8">
        <h1 class="text-3xl font-bold text-white mb-4 md:mb-0">Manage Parcs</h1>
        <div class="flex items-center space-x-4">
            <button onclick="openModal('add')" class="btn-primary text-white px-4 py-2 rounded-lg">Add Parc</button>
        </div>
    </header>

    <!-- Parcs Table -->
    <section class="card p-6">
        <c:if test="${empty parcs}">
            <p class="text-gray-400">No parcs found. Add a new parc to get started.</p>
        </c:if>
        <c:if test="${not empty parcs}">
                ...
                <table class="w-full text-left">
                    ...
                    <c:forEach var="parc" items="${parcs}">
                        <tr class="border-t border-gray-700 table-row">
                            <td class="text-white">${parc.parc_name}</td>
                            <td class="text-white">${parc.address}</td>
                            <td class="text-white">${parc.city}</td>
                            <td class="text-white">${parc.capacity}</td>
                            <td class="text-white">${parc.car_count}</td>
                            <td class="space-x-2">
                                <button onclick="openModal('edit', ${parc.parc_id}, '${parc.parc_name}', '${parc.address}', '${parc.city}', ${parc.capacity})" class="text-blue-400 hover:text-blue-300 font-medium">Edit</button>
                                <form action="parcs" method="post" style="display:inline;">
                                    <input type="hidden" name="action" value="delete">
                                    <input type="hidden" name="parcId" value="${parc.parc_id}">
                                    <button type="submit" class="text-red-400 hover:text-red-300 font-medium" onclick="return confirm('Are you sure you want to delete this parc?')">Delete</button>
                                </form>
                            </td>
                        </tr>
                    </c:forEach>
...
<!-- Modal for Add/Edit Parc -->
<div id="parcModal" class="modal">
    <div class="modal-content">
        <span class="close-btn" onclick="closeModal()">×</span>
        <h2 id="modalTitle" class="text-xl font-bold text-white mb-6"></h2>
        <form id="parcForm" action="parcs" method="post">
            <input type="hidden" name="action" id="modalAction">
            <input type="hidden" name="parcId" id="parcId">
            <div class="mb-5">
                <label class="form-label">Parc Name</label>
                <input type="text" name="parcName" id="parcName" class="input-field" required>
            </div>
            <div class="mb-5">
            ...
            <div class="mb-6">
                ...
                <input type="number" name="capacity" id="capacity" class="input-field" required>
            </div>
...
<script>
    function openModal(action, parcId = '', parcName = '', address = '', city = '', capacity = '') {
        const modal = document.getElementById('parcModal');
        const modalTitle = document.getElementById('modalTitle');
        const modalAction = document.getElementById('modalAction');
        const parcIdField = document.getElementById('parcId');
        const parcNameField = document.getElementById('parcName');
        const addressField = document.getElementById('address');
        const cityField = document.getElementById('city');
        const capacityField = document.getElementById('capacity');

        if (action === 'add') {
            modalTitle.textContent = 'Add New Parc';
            ...
            modalTitle.textContent = 'Edit Parc';
            modalAction.value = 'update';
            parcIdField.value = parcId;
            parcNameField.value = parcName;
